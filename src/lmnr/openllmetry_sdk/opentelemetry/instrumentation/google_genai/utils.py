...
def text_from_content_union(
    ...
    if isinstance(content, types.Content):
        ...
        return JOIN_PARTS_SEPARATOR.join(
            [_text_from_part_union(item) for item in content]
        )
        ...
        if "parts" in content:
            return JOIN_PARTS_SEPARATOR.join(
                [_text_from_part_union(item) for item in content.get("parts", [])]
            )
        else:
