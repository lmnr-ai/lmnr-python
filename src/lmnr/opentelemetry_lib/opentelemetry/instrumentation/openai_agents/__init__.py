...
_instruments = ("openai-agents >= 0.3.0",)
...
class LaminarAgentsTraceProcessor(TracingProcessor):
    ...
    def on_span_start(self, span: Any) -> None:
        ...
        if hasattr(lmnr_span, "set_attribute"):
            ...
            lmnr_span.set_attribute(
                "openai.agents.span.id", getattr(span, "span_id", "")
            )
        with self._lock:
    ...
    def force_flush(self):
        try:
            ...
            logger.warning(f"Failed to flush Laminar spans. Error: {e}")
...
def _map_span_type(span_data: Any) -> LaminarSpanType:
    kind = _span_kind(span_data)
...
def _apply_span_data(lmnr_span: Any, span_data: Any) -> None:
    ...
    if kind == "agent":
        ...
        _set_span_output(
            lmnr_span, output_text if output_text is not None else output_items
        )
