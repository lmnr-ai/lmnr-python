from .event_emitter import emit_response_events
from .utils import (
    dont_throw,
    should_send_prompts,
    to_dict,
...
from .span_utils import set_span_response_attributes, _set_span_chat_response
...
def process_chat_v1_streaming_response(span, event_logger, llm_request_type, response):
    ...
    try:
        for item in response:
            ...
            item_to_yield = item
            if getattr(item, "event_type", None) == "stream-end" and hasattr(
                item, "response"
            ):
...
@dont_throw
async def aprocess_chat_v1_streaming_response(
        ...
        for item in response:
            ...
            try:
                _accumulate_stream_item(
                    item, current_content_item, current_tool_call_item, final_response
                )
            except Exception:
                pass
            yield item_to_yield
...
@dont_throw
async def aprocess_chat_v2_streaming_response(
    span, event_logger, llm_request_type, response
):
    final_response = {
        "finish_reason": None,
...
def _accumulate_stream_item(
    ...
    if item_dict.get("type") == "message-start":
        ...
        new_thinking = (
            ((item_dict.get("delta") or {}).get("message") or {}).get("content") or {}
        ).get("thinking")
        if new_thinking:
        ...
        new_tool_plan = ((item_dict.get("delta") or {}).get("message") or {}).get(
            "tool_plan"
        )
        if new_tool_plan:
            existing_tool_plan = final_response["message"].get("tool_plan")
            final_response["message"]["tool_plan"] = (
                existing_tool_plan or ""
            ) + new_tool_plan
    elif item_dict.get("type") == "tool-call-start":
        new_tool_call_item = ((item_dict.get("delta") or {}).get("message") or {}).get(
            "tool_calls"
        )
        ...
        new_arguments = ((message.get("tool_calls") or {}).get("function") or {}).get(
            "arguments"
        )
        if new_arguments:
            existing_arguments = (current_tool_call_item.get("function") or {}).get(
                "arguments"
            )
            current_tool_call_item["function"]["arguments"] = (
                existing_arguments or ""
            ) + new_arguments
        ...
        final_response["finish_reason"] = (item_dict.get("delta") or {}).get(
            "finish_reason"
        )
