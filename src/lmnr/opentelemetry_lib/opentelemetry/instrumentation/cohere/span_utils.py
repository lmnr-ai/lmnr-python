    ...
    if should_send_prompts() and not should_emit_events():
        ...
        # client V1
            ...
            if system_message := kwargs.get("preamble"):
                ...
                _set_span_attribute(span, f"{GEN_AI_PROMPT}.0.content", system_message)
                user_message_index = 1
            _set_span_attribute(
                span, f"{GEN_AI_PROMPT}.{user_message_index}.role", "user"
            )
            _set_span_attribute(
                span,
                f"{GEN_AI_PROMPT}.{user_message_index}.content",
                kwargs.get("message"),
            )
        # client V2
            ...
            for index, message in enumerate(kwargs.get("messages")):
                ...
                _set_span_attribute(
                    span, f"{GEN_AI_PROMPT}.{index}.role", message_dict.get("role")
                )
                _set_span_attribute(
                    span,
                    f"{GEN_AI_PROMPT}.{index}.content",
        ...
        if kwargs.get("tools"):
            ...
            for index, document in enumerate(kwargs.get("documents", [])):
                _set_span_attribute(span, f"{GEN_AI_PROMPT}.{index}.role", "system")
                _set_span_attribute(span, f"{GEN_AI_PROMPT}.{index}.content", document)
...
def set_span_request_attributes(span, kwargs):
    ...
    _set_span_attribute(
        span, GEN_AI_REQUEST_TOP_P, kwargs.get("p", kwargs.get("top_p"))
    )
    _set_span_attribute(
        span, GEN_AI_REQUEST_TOP_K, kwargs.get("k", kwargs.get("top_k"))
    )

    if stop_sequences := kwargs.get("stop_sequences", []):
        _set_span_attribute(
            span, GEN_AI_REQUEST_STOP_SEQUENCES, dump_object(stop_sequences)
        )
