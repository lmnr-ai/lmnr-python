...
class AnthropicRolloutWrapper(RolloutInstrumentationWrapper):
    """
    Rollout wrapper specific to Anthropic instrumentation.
    ...
    def apply_anthropic_overrides(
        ...
        if "system" in overrides:
            ...
            # Anthropic sends system prompt as a separate 'system' kwarg
    ...
    def cached_response_to_anthropic(
        ...
        Tries raw response first, then falls back to reconstructing from output.
        """
    ...
    def wrap_create(
        ...
        logger.debug(f"Anthropic call at {span_path}:{current_index}")
    ...
    def _create_cached_stream(self, response: Message) -> Generator[Any, None, None]:
        """Yield a cached response as streaming events mimicking Anthropic's stream format."""
        ...
        from anthropic.types.raw_message_delta_event import MessageDeltaUsage
        ...
        yield RawMessageDeltaEvent(
            ...
            usage=MessageDeltaUsage(output_tokens=response.usage.output_tokens),
        )
