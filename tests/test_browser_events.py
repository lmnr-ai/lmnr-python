...
class TestBrowserEvents:
    ...
    @pytest.mark.asyncio
    async def test_async_browser_events(
        self, mock_sleep, mock_async_client, mock_async_page
    ):
        ...
        await send_events_async(
            mock_async_page, session_id, trace_id, mock_async_client
        )

        # Verify events were sent
        assert (
            mock_async_client._browser_events.send.called
        ), "No events were sent to Laminar"
    ...
    def test_multiple_pages(self, mock_sleep, mock_sync_client):
        ...
        assert (
            mock_sync_client._browser_events.send.call_count == 2
        ), "Events should be sent twice"
