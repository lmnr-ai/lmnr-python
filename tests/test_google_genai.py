...
def test_google_genai(exporter: InMemorySpanExporter):
    ...
    assert spans[0].attributes["gen_ai.completion.0.content"] == response.text
...
def test_google_genai_multiturn(exporter: InMemorySpanExporter):
    ...
    assert (
        adjective_span.attributes["gen_ai.completion.0.content"]
        == adjective_response.text
    )
    ...
    assert haiku_span.attributes["gen_ai.completion.0.content"] == haiku_response.text
