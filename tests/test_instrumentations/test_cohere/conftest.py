...
import cohere
import pytest
from lmnr.opentelemetry_lib.opentelemetry.instrumentation.cohere import (
    CohereInstrumentor,
)
from lmnr.opentelemetry_lib.opentelemetry.instrumentation.cohere.utils import (
    DO_TRACE_CONTENT,
)
...
def instrument_with_content(tracer_provider, event_logger_provider):
    os.environ.update({DO_TRACE_CONTENT: "True"})
    ...
    os.environ.pop(DO_TRACE_CONTENT, None)
    instrumentor.uninstrument()


@pytest.fixture(scope="function")
def instrument_with_no_content(tracer_provider, event_logger_provider):
    os.environ.update({DO_TRACE_CONTENT: "False"})
