...
class TestSyncLaminarClientTraces:
    ...
    def test_merge_metadata_success(self, sync_client):
        ...
        with patch.object(sync_client._LaminarClient__client, "post") as mock_post:
            ...
            sync_client.traces.merge_metadata(trace_id, {"score": 0.85, "reviewer": "alice"})
    ...
    def test_merge_metadata_accepts_int_trace_id(self, sync_client):
        trace_id_int = 0x12345678123456789ABC123456789ABC
        with patch.object(sync_client._LaminarClient__client, "post") as mock_post:
            mock_post.return_value = _ok_response()
            sync_client.traces.merge_metadata(trace_id_int, {"k": "v"})
        sent_id = mock_post.call_args.kwargs["json"]["traceId"]
        assert uuid.UUID(sent_id).int == trace_id_int

    def test_merge_metadata_404_does_not_raise(self, sync_client):
        with patch.object(sync_client._LaminarClient__client, "post") as mock_post:
            mock_post.return_value = _not_found_response()
            sync_client.traces.merge_metadata(uuid.uuid4(), {"k": "v"})
        mock_post.assert_called_once()

    def test_merge_metadata_500_raises(self, sync_client):
        with patch.object(sync_client._LaminarClient__client, "post") as mock_post:
            mock_post.return_value = _error_response()
            with pytest.raises(ValueError, match="Failed to merge trace metadata"):
                sync_client.traces.merge_metadata(uuid.uuid4(), {"k": "v"})

    def test_merge_metadata_empty_metadata_raises(self, sync_client):
        with pytest.raises(ValueError, match="non-empty"):
            sync_client.traces.merge_metadata(uuid.uuid4(), {})
...
class TestAsyncLaminarClientTraces:
    ...
    @pytest.mark.asyncio
    async def test_merge_metadata_success(self, async_client):
        trace_id = uuid.UUID("12345678-1234-5678-9abc-123456789abc")
        with patch.object(
            async_client._AsyncLaminarClient__client, "post", new_callable=AsyncMock
        ) as mock_post:
            mock_post.return_value = _ok_response()
            await async_client.traces.merge_metadata(
    ...
    @pytest.mark.asyncio
    async def test_merge_metadata_404_does_not_raise(self, async_client):
        with patch.object(
            async_client._AsyncLaminarClient__client, "post", new_callable=AsyncMock
        ) as mock_post:
            mock_post.return_value = _not_found_response()
            await async_client.traces.merge_metadata(uuid.uuid4(), {"k": "v"})
        mock_post.assert_called_once()

    @pytest.mark.asyncio
    async def test_merge_metadata_500_raises(self, async_client):
        with patch.object(
            async_client._AsyncLaminarClient__client, "post", new_callable=AsyncMock
        ) as mock_post:
            mock_post.return_value = _error_response()
    ...
    @pytest.mark.asyncio
    async def test_merge_metadata_empty_metadata_raises(self, async_client):
        with pytest.raises(ValueError, match="non-empty"):
            await async_client.traces.merge_metadata(uuid.uuid4(), {})
