...
def test_evaluate_with_human_evaluators_parameter(exporter: InMemorySpanExporter):
    """
    Test the evaluate function with human_evaluators parameter (sync version).
    """
    mock_eval_id = "00000000-0000-0000-0000-000000000000"
    ...
    with patch(
        ...
        # Patch the datapoints method and capture the saved datapoints
        saved_datapoints = []
        
        def capture_datapoints(eval_id, datapoints, group_name):
            saved_datapoints.extend(datapoints)
            return datapoints_response
            
        with patch(
            "lmnr.sdk.client.asynchronous.resources.evals.AsyncEvals.save_datapoints",
            side_effect=capture_datapoints,
        ) as mock_datapoints:

            # Run the evaluate function with human_evaluators parameter
            result = evaluate(
                data=[
                    {
                        "data": "test input",
                ...
                executor=lambda data: f"result: {data}",
                evaluators={
                    "similarity": lambda output, target: 0.75,
                },
                human_evaluators=[human_evaluator1, human_evaluator2],
                project_api_key="test",
            )
            ...
            # Verify that human evaluators are included in the datapoint
            final_datapoints = [dp for dp in saved_datapoints if hasattr(dp, 'human_evaluators')]
            assert len(final_datapoints) > 0
            
            final_datapoint = final_datapoints[0]
            assert len(final_datapoint.human_evaluators) == 2
