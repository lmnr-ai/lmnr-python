...
- `patch_temporal_worker` patches `Worker.create` (a static method); `patch_temporal_client` patches `Client.connect` (a classmethod). Both are called from `Laminar.initialize()` when `temporal_modules` dict is provided (`{"worker": temporal_worker_module, "client": temporal_client_module}`).
