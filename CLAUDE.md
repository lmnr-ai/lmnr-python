...
## opencode-ai instrument

- Mirrors the TS `OpencodeInstrumentation`: wraps `opencode_ai.resources.session.SessionResource.chat` and its async twin, appending a synthetic `{type: "text", text: "", synthetic: True, ignored: True, metadata: {lmnrSpanContext: ...}}` part to `parts` when a Laminar span context is active. The Opencode server forwards that metadata so the downstream LLM span re-parents onto the caller's trace.
- `TextPartInputParam` is a `total=False` `TypedDict` and Stainless's `maybe_transform` preserves unknown keys as-is, so `metadata`/`ignored`/`synthetic` survive serialization without needing a new type. Verify with `maybe_transform(params, SessionChatParams)` if the SDK ever changes.
- `_instruments = ("opencode-ai >= 0.1.0a0",)` — the pre-release spec is load-bearing: shipped versions are `0.1.0aN` alphas, and `>= 0.1.0` fails `BaseInstrumentor.instrument()`'s version check with a `DependencyConflict`, leaving `SessionResource.chat` unwrapped (silent failure visible only by checking `hasattr(...chat, "__wrapped__")`).
- Wrappers must copy `parts` (`[*parts, synthetic_part]`, not `parts.append(...)`): callers frequently reuse the same list across turns, and in-place mutation would compound a new synthetic part into every subsequent call.
- `LaminarSpanContext` serializes with snake_case keys (`trace_id`, `span_id`) — not `traceId`/`spanId` like the TS SDK. Tests asserting on the injected JSON must use snake_case. The `span_id` in the serialized context is a 32-hex-digit UUID whose last 16 hex digits are the OTel span id.
- Tests live in `tests/test_instrumentations/test_opencode/` and drive the real `opencode_ai` client over `httpx.MockTransport`, not a hand-rolled mock — this exercises the actual Stainless transform path and catches alias regressions (`model_id` → `modelID`).
