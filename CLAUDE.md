...
- When `PYDANTIC_AI` is requested and the user did not explicitly opt into the underlying provider SDKs (Anthropic, Bedrock, Cohere, Google GenAI, Groq, LiteLLM, Mistral, Ollama, OpenAI, Together, VertexAI), those provider instrumentors are auto-skipped to prevent double-instrumentation. pydantic_ai emits its own OTel GenAI spans via `InstrumentationSettings(version=5)`, so the raw HTTP-level instrumentors would create duplicates. The authoritative list lives in `_PYDANTIC_AI_CONFLICTING_INSTRUMENTS` in `src/lmnr/opentelemetry_lib/tracing/instruments.py` — update both together.
