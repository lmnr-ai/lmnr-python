...
- **Langfuse wins over both deepagents and pydantic_ai** when they are all installed. Priority ladder is `langfuse > deepagents > pydantic_ai`. The bridge's entire job is to dual-export spans through Langfuse's own auto-patchers (`langfuse.openai`, `langfuse.langchain`, `@observe`) to both Langfuse *and* Laminar — running Laminar's raw-provider or pydantic_ai instrumentors alongside would double-cover the same LLM call. Deepagents still contributes its `DEFAULT` root + `TOOL` spans (Langfuse doesn't emit those); its LLM children come from Langfuse's auto-patchers riding through the bridge, not from Laminar's raw-provider instrumentors. When `langfuse_active`, `init_instrumentations` strips `_LANGFUSE_PROVIDER_CONFLICTS` AND `{Instruments.PYDANTIC_AI}` regardless of deepagents. Callers who want the raw-provider instrumentors alongside Langfuse can pass an explicit `instruments` set.
