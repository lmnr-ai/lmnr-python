...
- `patch_temporal_worker` patches `Worker.__init__`; `patch_temporal_client` patches `Client.connect`. Both are called from `Laminar.initialize()` when `temporal_modules` is truthy. Pass `temporal_modules=True` for zero-config auto-patching — the SDK imports `temporalio.worker` / `temporalio.client` itself via `importlib.import_module` and silently skips if they are not installed. Pass `temporal_modules={"create_activity_span": False}` to suppress per-activity spans.
