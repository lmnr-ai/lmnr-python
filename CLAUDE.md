...
- `Instruments.LANGFUSE` is **auto-enabled by default** when `langfuse >= 3.0` is installed. When auto-enabled, **every other Laminar auto-instrumentor is disabled** — the default instrument set is reduced to `{Instruments.LANGFUSE, Instruments.OPENTELEMETRY}` (the latter is a DataDog-SpanContext patch that emits no spans). The Langfuse bridge dual-attaches Laminar's `SpanProcessor` to every Langfuse `TracerProvider`, so Langfuse-emitted spans still reach Laminar — running any of Laminar's raw-provider / framework / agent instrumentors on top would double-cover what Langfuse already traces. `_LANGFUSE_PROVIDER_CONFLICTS` is kept only for blocked-langfuse tests; the main default-set reduction is now a single replacement (`instruments = {LANGFUSE, OPENTELEMETRY}`), not a subtraction.
...
- **When Langfuse auto-enables, Laminar defers completely to it.** No Laminar instrumentor runs alongside — not raw providers, not deepagents, not pydantic_ai. The rationale is that the bridge's whole job is to route Langfuse-emitted spans into Laminar; running any Laminar-side instrumentor on top would double-cover the same call. Callers who want specific Laminar instrumentors alongside Langfuse can opt back in by passing an explicit `instruments` set to `Laminar.initialize`.
